...
export const metadata: Metadata = {
  ...
  description: 'Explora nuestro catálogo de rodamientos, piñones, correas y ferretería industrial. Precios en COP, envíos a toda Colombia.',
}
...
interface ProductRow {
  ...
  comparePrice: string | null; stock: number; categoryName: string | null
}
...
export default async function ProductosPage({
  ...
  const params = new URLSearchParams({ page, pageSize: '24' })
  ...
  try {
    ;[result, categories] = await Promise.all([
      publicGet<PaginatedResponse<ProductRow>>(`/api/products?${params}`),
      publicGet<Category[]>('/api/categories/flat'),
    ])
  } catch {}
